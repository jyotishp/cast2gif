...
from cast2gif import AsciiCast
from cast2gif.asciicast import VERSION_NAME
...
def main(argv=None):
    ...
    parser.add_argument(
        ...
        default=None,
        required=True,
        help="The path for the output GIF file, or '-' for STDOUT (default is the input filename plus '.gif', or STDOUT if the input file is STDIN)",
    )
    parser.add_argument(
        "--force",
        action="store_true",
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help="The maximum amount of idle time that can occur between terminal events; by default this is read from the AsciiCast input, or set to zero if none is specified in the input; if provided, this option will override whatever is specified in the input",
    )
    parser.add_argument(
        "--loop",
        type=int,
